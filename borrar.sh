...
echo "Cantidad maxima permitida: ${CANT_MAX}. Hay ${CANT_ARCHIVOS} archivos."
...
then
    ...
    ls ${DIR_BACKUPS} -l | sort | head -n $(( ${CANT_ARCHIVOS} - ${CANT_MAX} )) | cut -d ' ' -f 9 | xargs printf -- "   -> ${DIR_BACKUPS}/%s \n"
    ls ${DIR_BACKUPS} -l | sort | head -n $(( ${CANT_ARCHIVOS} - ${CANT_MAX} )) | cut -d ' ' -f 9 | xargs printf -- "$(date)    -> ${DIR_BACKUPS}/%s \n" >> ${LOG_DIR}
    ls ${DIR_BACKUPS} -l | sort | head -n $(( ${CANT_ARCHIVOS} - ${CANT_MAX} )) | cut -d ' ' -f 9 | xargs printf -- "${DIR_BACKUPS}/%s " | xargs rm
else
